<div class="users form form-margin-bottom">
    <?php
    echo $this->Form->create('User', array( 'type' => 'file', 'inputDefaults' => array( 'label' => false, 'div' => false ), 'class' => 'user-form' ));

    $sub_form = $this->InventoryLookup->user_form_elements($section);
    echo $this->element('Forms/User/' . $sub_form['form']."_permission", array( 'edit' => $edit, 'legend' => $legend ));
    ?>
    ...
    <?php }
        ...
        <div class="align-left align-top-margin">
            <?php echo $this->Html->link('Cancel', array( 'action' => 'user_permission_detail', $id, $section ), array( 'data-target' => '#MainContent', 'class' => 'ajax-sub-content btn btn-success btn-padding', 'label' => false, 'value' => 'Cancel' )); ?>
        </div>
    <?php } ?>
    <?php
    echo $this->Form->end();
