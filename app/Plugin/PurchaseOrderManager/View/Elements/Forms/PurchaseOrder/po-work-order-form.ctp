...
if ($section == "work-order-po") {
  $all_items = $this->InventoryLookup->ListQuoteItems($quote_id);
    ...
    <table class='table-form-big po-supplier-table-mid-width'>
      <tbody>
        <tr>
          ...
          <td colspan="2">
						<?php echo $this->Form->input('supplier_id', array('placeholder' => 'Name', 'class' => 'medium-input-select required form-select supplier-select', "options" => $this->InventoryLookup->Supplier(), 'empty' => true,)); ?>
          ...
          <td colspan="3">
            ...
            <?php echo $this->Form->input('quote_id', array('type' => 'hidden', 'value' => $quote_id)); ?>
            <?php echo $this->Form->input('work_order_id', array('type' => 'hidden', 'value' => $work_id)); ?>
          </td>        
        </tr>
    </table>
    <table id='search_div' style="display:<?= isset($_GET['search']) ? "block" : "none" ?>" class="table-form-big table-form-big-margin po-supplier-table-mid-width">
      ...
      <tr>
        ...
        <td class="quote-title">          
          <?php echo $quoteInfo[0]['WorkOrder']['work_order_number']; ?>
        </td> 
        <th class="width-change"><label for="SupplierFirstName">Sales Persons: </label></th>
        <td colspan="2">
					<?php 
						$sales = unserialize($quoteInfo[0]['Quote']['sales_person']); 
						$cnt = count($sales);
						$j = 1;
						for($i = 0; $i<$cnt; $i++){
      ...
      <tr>       
        ...
        <td colspan="3" class="quote-address">
          <?php
          echo $this->InventoryLookup->address_format($quoteInfo[0]['Quote']['address'], $quoteInfo[0]['Quote']['city'], $quoteInfo[0]['Quote']['province'], $quoteInfo[0]['Quote']['country'], $quoteInfo[0]['Quote']['postal_code']);
          ?>
        </td>
      </tr>      
      </tbody>
...
<!--            <label class="table-data-title">Payment Information</label>-->
        ...
        <tr>
					...
					<td colspan="2">
					<?php echo $this->Form->input('shipment_date', array('placeholder' => 'DD/MM/YYYY', 'type' => 'text', 'class' => 'dateP required', 'value' => $this->Util->formatDate($quoteInfo[0]['Quote']['est_shipping']))); ?>
					</td>  
					<th>
...
<script type="text/javascript">
	...
	$('.supplier-select').bind("change", function(){
