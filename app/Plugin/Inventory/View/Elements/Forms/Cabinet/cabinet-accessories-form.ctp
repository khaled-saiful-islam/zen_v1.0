<div id="cabinet_items" class="tab-pane">
    <fieldset>
        <legend <?php if( $edit ) { ?>class='inner-legend'<?php } ?>>Edit Cabinet Accessories</legend>
        <div class="cabinets-items">
            <table class="table-form-big" style="width: 70%;">
                <thead>
                    <tr>
                        <th>Item (accessories) Code</th>
                        <th>Description</th>
                        <th>Department</th>
                        <th>Cost</th>
                        <th>Quantity</th>
...
//              }
                            ...
                            echo $this->Form->input("CabinetsItem.-1.item_id", array(
                                "class" => "item_id user-input form-select-ajax-item-accessories form-select-ajax-item-cabinet-item wide-input",
                                "label" => false,
                                "type" => 'hidden',
                    ...
                    if( !empty($this->data['CabinetsItem']) ) {
                        foreach( $this->data['CabinetsItem'] as $index => $value ) {
                            if( !$value['accessories'] )
                                continue; // skip if not accessories
                            ...
                            <tr class="clone-row">
                                <td>
                                    ...
                                    echo $this->Form->input("CabinetsItem.{$index}.item_id", array(
                                        "class" => "item_id user-input form-select-ajax-item-accessories form-select-ajax-item-cabinet-item wide-input required",
                                        "label" => false,
                                        "type" => 'hidden',
                                    ));
                                    ?>
                    ...
                    <tr>
                        <td colspan="6">
                            <input type="button" class="btn btn-info add-more" value="Add An Accessories" />
                        </td>
                    </tr>
                </tbody>
            </table>
