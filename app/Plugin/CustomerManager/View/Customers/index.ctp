...
if ($paginate) {
  ...
  <div class="customers index">
    <fieldset class="content-detail">
      ...
      <div id="search_div">
        <?php echo $this->Form->create('Customer', array('url' => array_merge(array('action' => 'index'), $this->params['pass']), 'inputDefaults' => array('label' => false, 'div' => false))); ?>
      ...
      <table cellpadding="0" cellspacing="0" class="table table-bordered table-hover listing">
        <thead>
          <tr class="grid-header">
            ...
            <th class="sales_rep">Sales Rep</th>
            ...
            <tr>
              <td>
                ...
                if (!empty($customer['Customer']['last_name'])) {
                  $customer_name = h($customer['Customer']['last_name']) . ', ' . h($customer['Customer']['first_name']);
                } else {
                  $customer_name = h($customer['Customer']['first_name']);
                }
...
<?php } else { ?>
  <table cellpadding="0" cellspacing="0" class="table table-bordered table-hover listing">
    <thead>
      <tr class="grid-header">
        <th class="name"><?php echo h('Name'); ?></th>
        <th class="email"><?php echo h('E-mail'); ?></th>
        <th class="phone"><?php echo h('Cell'); ?></th>
        <th class="phone"><?php echo h('Phone'); ?></th>
        <th class="city"><?php echo h('City'); ?></th>
    ...
    <tbody>
      <?php foreach ($customers as $customer): ?>
        <tr>
          ...
          <td class="email"><?php echo h($customer['Customer']['email']); ?>&nbsp;</td>
          <td class="phone"><?php echo h($customer['Customer']['cell']); ?>&nbsp;</td>
          <td class="phone"><?php echo h($customer['Customer']['phone']); ?>&nbsp;<?php if ($customer['Customer']['phone_ext']) echo 'Ext: ' . h($customer['Customer']['phone_ext']); ?></td>
          <td class="city"><?php echo h($customer['Customer']['city']); ?>&nbsp;</td>
          <td class="sales_rep">
            <?php
//            App::uses("CustomerSalesRepresentetives", "CustomerManager.Model");
//            $sales = new CustomerSalesRepresentetives();
