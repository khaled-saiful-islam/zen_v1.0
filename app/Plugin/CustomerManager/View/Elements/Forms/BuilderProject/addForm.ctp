<h4>Add Project</h4>
<?php
		echo $this->Form->create('BuilderProject', array('url' => array('action' => 'addProject', $customer_id), 'inputDefaults' => array('label' => false, 'div' => false), 'class' => 'ajax-form-submit project_form'));
?>
			...
			<td>
				<?php
					echo $this->Form->input('BuilderProject.contact_person_phone', array(
						'class' => 'input-medium phone-mask required',
...
<!--  <div class="align-left align-top-margin"><a href="<?php echo urldecode( $this->Html->url( array('controller' => 'builders', 'action' => 'detail', $customer_id.'#project') ) ); ?>">TEST</a><?php //echo $this->Html->link('Cancel', array('controller' => 'builders', 'action' => 'detail', $customer_id.'#project'), array( 'escape' => false, 'class' => 'btn btn-success btn-padding', 'title' => __('Cancel'))); ?></div>-->
  <div class="align-left align-top-margin"><input type="button" value="Cancel" onclick="newlink()"></div>
	<div class="clear"></div>
	<?php echo $this->Form->end(); ?>

<script>
	$(".project_form").validate({ignore: null});
  $(".project_form").ajaxForm({url: $(this).attr('action'), type: 'post',  target: '#sub-project-list'});
	function newlink(){
		window.location.assign('<?php echo "http://{$_SERVER['SERVER_NAME']}{$this->webroot}customer_manager/builders/detail/{$customer_id}#project"; ?>')
  }
</script>