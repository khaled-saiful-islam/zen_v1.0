...
<page>
    ...
    <div class="tab-content">
        ...
        <div style="width: 100%; min-height: 200px; margin-top: 20px;">
            <table style="width: 100%;">
                <tr>
                    <td style="width: 50%;"><h4 style="margin: 0px;">Description of Total</h4></td>
                    <td style="width: 50%;"><h4 style="margin: 0px;">Matching item on report</h4></td>
                </tr>
                <?php
                if( !empty($report_items) ) {
                    foreach( $report_items as $report_item ) {
                        if( ($report_item['quantity'] % 2) == 0) {
                            if( $report_item['length'] >= 250 ) {
                                ?>
                                <tr>
                                    <td style="width: 50%;">
                                        <div>
                                            <?php
                                            if( $report_item['length'] >= 250 && $report_item['length'] < 350 ) {
                                                echo "250mm Slides (Pair)";
                                            }
                                            elseif( $report_item['length'] >= 350 && $report_item['length'] < 400 ) {
                                                echo "350mm Slides (Pair)";
                                            }
                                            elseif( $report_item['length'] >= 400 && $report_item['length'] < 450 ) {
                                                echo "400mm Slides (Pair)";
                                            }
                                            elseif( $report_item['length'] >= 450 && $report_item['length'] < 500 ) {
                                                echo "450mm Slides (Pair)";
                                            }
                                            elseif( $report_item['length'] >= 500 ) {
                                                echo "500mm Slides (Pair)";
                                            }
                                            ?>
                                        </div>
                                    </td>
                                    <td style="width: 50%;">
                                        <div>
                                            <?php echo $report_item['code'] . "&nbsp;&nbsp;&nbsp;&nbsp;" . $report_item['quantity'] / 2; ?>
                                        </div>
                                    </td>
                                </tr>
                                <?php
        ...
        if( !empty($report_items) ) {
            ?>
            <h2 class="text-center red" style="margin-top: 20px; text-align: center; width: 100%;">*** <?php echo $this->InventoryLookup->InventoryLookupReverse($quote['Quote']['drawer']); ?> *** </h2>
            <?php
        }
        ?>
    </div>
