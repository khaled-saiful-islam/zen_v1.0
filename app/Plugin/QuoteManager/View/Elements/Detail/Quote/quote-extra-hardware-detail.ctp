<?php if ($edit) { ?>
  <div class="detail actions">
    <?php echo $this->Html->link('Add/Edit', array('action' => EDIT, $quote['Quote']['id'], 'extra-hardware'), array('data-target' => '#quote-extra-hardware-detail', 'class' => 'ajax-sub-content btn btn-success', 'title' => __('Edit'))); ?>
  </div>
<?php } ?>
<fieldset>
  <?php 
  $quote['QuoteExtraHardware'] = array();
  $quote['QuoteExtraHardware'] = $this->InventoryLookup->ListQuoteAllItem(null, $quote['Quote']['id'], 'Extra Hardware');
  
  if ($quote['QuoteExtraHardware']) { ?>
    <table cellpatding="0" cellspacing="0" class="table table-bordered table-hover listing" >
      ...
      <tbody>
        ...
        foreach ($quote['QuoteExtraHardware'] as $item):
          $count++;
