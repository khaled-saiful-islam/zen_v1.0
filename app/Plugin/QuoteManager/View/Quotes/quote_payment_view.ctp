          ...
          <tr>
            ...
            <td style="width:20px">
              <?php 
							...
							?>
              <?php
								$quote_status = $this->InventoryLookup->QuoteStatus($Quote_data['Quote']['id']);
								if($quote_status != 'Review' && $Quote_data['Quote']['delete'] == 0)
								echo $this->Html->link(__('', true), array('action' => 'delete_single_file_payment', $Quote_data['Quote']['id'], $upload['UploadPayment']['id']), array('class' => 'icon-trash ask-delete', 'data-button' => "Delete {$upload['UploadPayment']['filename']}", 'data-msg' => "Do you really want to delet the file '{$upload['UploadPayment']['filename']}'")); 
							?>
            </td>
          </tr>
          <?php
...
<?php echo $this->element('Forms/Quote/payment-info-form', array( 'quote' => $Quote_data, 'edit' => true )); ?>
